...
const Admin = ()=>{
   ...
   const [isOPen, setIsOpen] = useState(false);
    ...
    // const userId = user?.email
      return <div className="flex flex-col justify-center items-center gap-4 p-3 ">
        {/* <h1>ADMIN</h1> */}
        ...
        >
          ...
          <Button value="Feedback" onClick={()=>setView('feedback')} cl={`mr-2 text-xl  hover:bg-gray-400 w-fit ${
                view === "feedback" ? "bg-gray-600 text-white" : ""
              }  `}/>
        </div>
      </div>
        ...
        <table className="  border-collapse border border-gray-200 w-full mx-28">
            ...
            <tbody>
               {renderArticle().map((user,index)=>(
                <tr key={index}>
                    <td className="border border-gray-300 px-4 py-2 text-center align-middle">{user}</td>
