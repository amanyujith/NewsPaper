...
import { BriefcaseBusiness, Cpu, HeartPulse, House, Telescope, Trophy } from 'lucide-react';
    ...
    <div className="">
      <div className="flex bg-gray-700 text-white gap-5">
        <nav className="flex justify-evenly flex-wrap gap-4 w-full p-3">
          {[
            { id: 'all', label: 'ALL NEWS', icon: <House />, endpoint: 'everything?q=general' },
            { id: 'Science', label: 'SCIENCE', icon: <Telescope />, endpoint: 'everything?q=science' },
            { id: 'Sports', label: 'SPORTS', icon: <Trophy />, endpoint: 'everything?q=sports' },
            { id: 'Tech', label: 'TECHNOLOGY', icon: <Cpu />, endpoint: 'everything?q=technology' },
            { id: 'Health', label: 'HEALTH', icon: <HeartPulse />, endpoint: 'everything?q=health' },
            { id: 'Business', label: 'BUSINESS', icon: <BriefcaseBusiness />, endpoint: 'everything?q=business' },
            ...
            >
              {isClicked === button.id ? button.icon : (
                <>
